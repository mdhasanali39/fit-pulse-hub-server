...
app.use(
  cors({
    origin: ["http://localhost:5173","https://console.firebase.google.com/project/fit-pulse-hub-web-app/overview","https://fit-pulse-hub-web-app.web.app"],
    credentials: true,
  })
);
...
async function run() {
  try {
    ...
    const timeslotCollection = client.db("fitpulsehubDb").collection("timeslots");
    const membershipCollection = client.db("fitpulsehubDb").collection("memberships");
    const subscriberCollection = client.db("fitpulsehubDb").collection("subscribers");
    ...
    app.get("/api/v1/trainer/:id", async(req,res)=>{
    ...
    app.get("/api/v1/count-subscribers-memberships", async(req,res)=>{
      const totalSubscribersCount = await subscriberCollection.estimatedDocumentCount();
      const totalPaidMembersCount = await membershipCollection.estimatedDocumentCount()
    ...
    app.get("/api/v1/total-payment-stat", async(req,res)=>{
      const result1 = await membershipCollection.aggregate([
        {
          $group: {
            _id:null,
      ...
      const membersPaymentTotal = result1.length >0 ? result1[0].totalPayment : 0
      ...
      const adminPaymentTotal = result2.length >0? result2[0].totalPayment: 0

      res.send({membersPayment:membersPaymentTotal,adminPayment:adminPaymentTotal})
    })

    // get last six payments transaction - made by members 
    app.get("/api/v1/payments/last-six", async(req,res)=>{
      try {
        const result = await membershipCollection.find().sort({timeStamp: -1}).limit(6).toArray()
        res.send(result)
      } catch (err) {
        console.log(err);
      }
    ...
    app.post("/api/v1/newsletter-subscriber/:email", async(req,res)=>{
      ...
      const subscriberData = {
        ...subscriber, created_at: Date.now()
      }
    ...
    app.patch("/api/v1/update-trainer/:email/:id", async(req,res)=>{
      ...
      const updatedTrainer = {
        $set: {
          status: status,salary:"pending"
        }
      }
      const trainerUpdated = await trainerCollection.updateOne(filter,updatedTrainer)
      res.send({trainerUpdated, userUpdated})
    })
    // update trainer salary status to received 
    app.patch("/api/v1/update-trainers", async(req,res)=>{
      
      const salaryStatus = req.body?.salary;
      const filter = {salary:"pending"}
      const updatedSalaryStatus = {
        $set: {
          salary:salaryStatus
        }
      }
      const result = await trainerCollection.updateMany(filter,updatedSalaryStatus)
      res.send(result)
    })
